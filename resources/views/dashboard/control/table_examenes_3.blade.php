                    ...
                    <td class="text-center" style="width: 5%;">
                        <div class="btn-group">
                            ...
                            <button type="button" class="btn btn-primary btn-sm"
                                    wire:click="editUroanalisis({{ $examen->id }})"
                                {{--@if(!comprobarPermisos('paciante.edit')) disabled @endif--}} >
                                <i class="fas fa-edit"></i>
                            </button>

                            <button type="button" class="btn btn-primary btn-sm"
                                    wire:click="destroyUroanalisis({{ $examen->id }})"
...
<div class="col-6 table-responsive" xmlns:wire="http://www.w3.org/1999/xhtml">
    <form wire:submit.prevent="saveUrocultivos">
        <table class="table {{--table-striped--}} table-bordered">
            <thead>
            <tr class="text-navy">
            ...
            <tbody>
            @foreach($listarUrocultivos as $examen)
                <tr>
                    ...
                    <td class="text-center">
                        <div class="btn-group">
                            ...
                            <button type="button" class="btn btn-primary btn-sm" wire:click="editUrocultivos({{ $examen->id }})"
                                {{--@if(!comprobarPermisos('paciante.edit')) disabled @endif--}} >
                                <i class="fas fa-edit"></i>
                            </button>

                            <button type="button" class="btn btn-primary btn-sm" wire:click="destroyUrocultivos({{ $examen->id }})"
                                @if(!comprobarPermisos()) disabled @endif >
                                <i class="fas fa-trash-alt"></i>
                            </button>
            ...
            @if($paciente_id)
                ...
                <tr class="table-primary">
                    <td>
                        <div class="form-group">
                            <div class="input-group mb-3">
                                <input type="date" class="form-control" wire:model.defer="ex4_fecha"
                                       placeholder="FECHA">
                                @error('ex4_fecha')
                                <span class="col-sm-12 text-sm text-bold text-danger">
                                <i class="icon fas fa-exclamation-triangle"></i>
                    ...
                    <td>
                        <div class="form-group">
                            <div class="input-group mb-3">
                                <input type="text" class="form-control" wire:model.defer="ex4_detalles"
                                       placeholder="Detalles">
                                @error('ex4_detalles')
                                <span class="col-sm-12 text-sm text-bold text-danger">
                                    <i class="icon fas fa-exclamation-triangle"></i>
